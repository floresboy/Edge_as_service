...
param (
    ...
    [string]$hivemqVersion = "Edge_latest",
    # Default HiveMQ installation folder if not provided
    [string]$installationFolder = "C:\hivemq-edge"
)
...
$links = @{
    ...
    HiveMQ_Link = "https://www.hivemq.com/releases/hivemq-edge-latest.zip"
    NSSM_Link = "https://nssm.cc/ci/nssm-2.24-101-g897c7ad.zip"
    MQTTCLI_Link = "https://github.com/hivemq/mqtt-cli/releases/download/v4.37.0/mqtt-cli-4.37.0-win.zip"
}
...
try {
    $downloadPath = "$tempPath\hivemq-edge.zip"
    ...
    if (Test-Path $extractedFolder) {
        Write-Host "HiveMQ $hivemqVersion extraction was successful! to $extractedFolder" -ForegroundColor Green
    } else {
...
Write-Host "4) Creating HiveMQ edge basic configuration file..."
$configContent = @"
<?xml version="1.0"?>
<!--
...
Write-Host "9) Creating HiveMQ Edge service..."
try {
    ...
    $serviceCommands = @(
        ...
        @{Arguments = "set HiveMQEdgeService Description HiveMQ Edge MQTT Broker"; Description = "Setting service Description..."},
        @{Arguments = "set HiveMQEdgeService ObjectName LocalSystem"; Description = "Setting User Account under which the service runs..."},
        @{Arguments = "set HiveMQSerHiveMQEdgeServicevice Type SERVICE_WIN32_OWN_PROCESS"; Description = "Setting service Type..."},
    ...
    Write-Host "10) Starting HiveMQ edge service..."
